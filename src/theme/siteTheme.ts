...
export const DEFAULT_THEME = 'tri-sunset';
export const DEFAULT_PALETTE = 'adobe';
...
export type SiteTheme = LegacyTheme | PaletteTheme;
...
function isPaletteTheme(theme: SiteTheme): theme is PaletteTheme {
  return 'palette' in theme;
}

function clearPaletteOverrides(root: HTMLElement): void {
...
export function applySiteTheme(themeKey: string): void {
  const theme = siteThemes[themeKey];
...
export function initSiteTheme(): void {
  ...
  const themeKey = savedTheme && siteThemes[savedTheme] ? savedTheme : DEFAULT_THEME;
